...
public class UserDo {
    ...
    private List<UserData> convertToTree(Cursor cursor) {
        ...
        for (int i = 0; i < resultCounts; i++) {
            ...
            tree.setRemark(cursor.getString(cursor.getColumnIndex("remark")));
            tree.setUrl_icon(cursor.getString(cursor.getColumnIndex("url_icon")));
            tree.setUrl(cursor.getString(cursor.getColumnIndex("url")));
...
//        sqLiteDatabase.close();
    ...
    //添加内容
    public  void addSql(String title, String remark, String url_icon ,String url) {
        sqLiteDatabase =  mySQLite.getWritableDatabase();
        ContentValues c = new ContentValues();
        c.put("title",title);
        c.put("remark",remark);
        c.put("url_icon",url_icon);
        c.put("url",url);
